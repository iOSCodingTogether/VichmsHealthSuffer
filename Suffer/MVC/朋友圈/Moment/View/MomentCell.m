...
- (void)setMoment:(Moment *)moment
{
    ...
    if ([moment.text length]) {
        ...
        static NSMutableDictionary * attTempDic;
        if(!attTempDic){
            attTempDic = [NSMutableDictionary dictionary];
        }
        NSString *text = [NSString stringWithFormat:@"%@",moment.text];
        NSAttributedString *attrStr = [attTempDic objectForKey:text];
        if(!attrStr){
        attrStr = [[NSAttributedString alloc] initWithData:[text dataUsingEncoding:NSUnicodeStringEncoding] options:@{NSDocumentTypeDocumentAttribute:NSHTMLTextDocumentType} documentAttributes:nil error:nil];
            [attTempDic setObject:attrStr forKey:text];
        }
