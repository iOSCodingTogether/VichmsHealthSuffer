...
#import "ExpertSearchVC.h"
#import "DepartmentPageRequestModel.h"
#import "DoctorViewController.h"

@interface ExpertSearchVC ()< UITableViewDelegate, UITableViewDataSource,UISearchBarDelegate>
@property (nonatomic, strong) UISearchBar *mSearchBar;

@property (nonatomic, strong) DepartmentPageResultModel * departmentModel;     //
...
-(void)request:(BOOL)isLoadMore{
    DepartmentPageRequestModel *model = [DepartmentPageRequestModel new];
    model.pageSize = 10;
    ...
    if(!isLoadMore){
        ...
        model.pageNo = [model nextPageNoForCurDataCount:self.departmentModel.list.count];
    }
    model.search_LIKE_department=self.mSearchBar.text;
    [BaseRequest requestWithRequestModel:model ret:^(BOOL success, __kindof BaseResultModel *dataModel, NSString *jsonObjc) {
        if(dataModel.success){
            if(!isLoadMore){
                self.departmentModel = dataModel;
            }
            else{
                NSMutableArray *list = [NSMutableArray array];
                [list addObjectsFromArray:self.departmentModel.list];
                self.departmentModel = dataModel;
                [list addObjectsFromArray:self.departmentModel.list];
                self.departmentModel.list = list;
            }
            [self reloadData];
        }
...
//    }
    ...
    for (NSInteger i=0;i<self.departmentModel.list.count;i++) {
        DepartmentPageResultSubModel *submodel = self.departmentModel.list[i];
        NSString *str = submodel.department;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    DepartmentPageResultSubModel *submodel = self.departmentModel.list[indexPath.row];

    DoctorViewController *vc = [DoctorViewController new];
    vc.departmentId = submodel.id;
